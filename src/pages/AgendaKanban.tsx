...
export default function AgendaKanban() {
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      // ORDENAÇÃO: Respeita o clique na tela, com agrupamento primário inteligente
      const dadosOrdenados = [...filtered].sort((a, b) => {
        // 1. Ordenação Secundária (A que foi clicada na tela)
        if (sortConfig) {
          let valA = a[sortConfig.key];
          let valB = b[sortConfig.key];
          if (sortConfig.key === 'status_global' || sortConfig.key === 'status') {
            valA = formatarStatus(a.status_global || a.status);
            valB = formatarStatus(b.status_global || b.status);
          }
          if (!valA) valA = "";
          if (!valB) valB = "";
        ...
        // 2. Primazia do Agrupamento Padrão (Responsável)
        const respA = a.responsavel || a.lider_card || "Sem Responsável";
        const respB = b.responsavel || b.lider_card || "Sem Responsável";
        if (respA < respB) return -1;
        if (respA > respB) return 1;
        
        // 3. Status e Datas
        const stA = formatarStatus(a.status_global || a.status);
        const stB = formatarStatus(b.status_global || b.status);
        const ordemA = pesoStatus[stA] || 99;
        const ordemB = pesoStatus[stB] || 99;
        if (ordemA !== ordemB) return ordemA - ordemB;
        
        return new Date(a.data_entrada || a.created_at || 0).getTime() - new Date(b.data_entrada || b.created_at || 0).getTime();
      });

      const tableColumn = ["Entrada", "Previsão", "Conclusão", "Solicitante", "Projeto/Processo", "Responsável", "Tarefa Atual", "Status", "Resumo/Obs"];
      const tableRows: any[] = [];
      let grupoAtual = null;

      dadosOrdenados.forEach(item => {
        // Define o agrupador dinâmico baseado na ordenação da tela (Fallback: Responsável)
        let valGrupo = item.responsavel || item.lider_card || "Sem Responsável";
        let labelGrupo = "Responsável";

        if (sortConfig) {
          if (sortConfig.key === 'responsavel' || sortConfig.key === 'lider_card') { valGrupo = item.responsavel || item.lider_card || "Sem Responsável"; labelGrupo = "Responsável"; } 
          else if (sortConfig.key === 'solicitante') { valGrupo = item.solicitante || "Sem Solicitante"; labelGrupo = "Solicitante"; } 
          else if (sortConfig.key === 'status_global' || sortConfig.key === 'status') { valGrupo = formatarStatus(item.status_global || item.status); labelGrupo = "Status"; } 
          else if (sortConfig.key === 'titulo' || sortConfig.key === 'projeto_processo') { valGrupo = item.titulo || item.projeto_processo || "Sem Projeto/Processo"; labelGrupo = "Projeto / Processo"; }
        }
        ...
        tableRows.push([
          formatarData(item.data_entrada || item.created_at), 
          formatarData(item.vencimento || item.previsao_prazo), 
          formatarData(item.data_conclusao),
          item.solicitante || "-", 
          item.titulo || item.projeto_processo || "-", 
          item.responsavel || item.lider_card || "-",
          item.coluna_nome || item.tarefa_atual || item.etapa || "-", 
          formatarStatus(item.status_global || item.status), 
          item.descricao || item.resumo_observacoes || "-"
        ]);
      });
      ...
      autoTable(doc, {
        ...
        body: tableRows,
        startY: 40,
        // MARGEM REFORÇADA: Limite exato de desenho para não esmagar o rodapé nem o cabeçalho
        ...
        didDrawPage: function (data) {
          ...
          // ==========================================
          // CAIXAS BRANCAS (Blindagem de fundo da tabela)
          // ==========================================
